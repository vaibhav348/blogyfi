...
const BlogSection = () => {
    ...
    return (<section className='bg-gradient-to-r from-pink-200 to-orange-200 py-6'>
        ...
        <div className='md:px-10  px-4 md:px-auto w-fit flex flex-col md:flex-row gap-3 lg:gap-8 justify-center '>
            ...
            <Link
  to={`/blog/${blogs[1]?._id}`}
  className='md:w-[40%] h-fit py-3 mx-10 flex mb-4 flex-col gap-3 w-fit rounded-2xl shadow-md mx-auto bg-white transition-transform transform duration-300 hover:scale-105'
>
  ...
  <img
    ...
    alt='blogs thumbnails'
    className='w-[95%] rounded-2xl shadow-1xl m-auto'
  />
  <div className='flex gap-2 mx-5 py-1'>
    {blogs[1]?.tags.map((tag, i) => (
      <span
        key={i}  // Fixed typo from 'kay' to 'key'
        className='px-2 py-1 text-gray-900 text-xs md:text-sm bg-gray-200 rounded-md font-semibold shadow-md capitalize'
      >
        {tag}
      </span>
    ))}
  ...
  <div className='mx-5 flex justify-start items-center gap-2'>
    <img
      src={blogs?.[1]?.profileImage ? blogs[1].profileImage : 'https://img.icons8.com/?size=100&id=x2tr2g6eXjMc&format=png&color=000000'}
    ...
    <div>
      ...
      <p className='text-xs font-bold uppercase text-gray-700 font-sans'>
        {convertDate(blogs[1]?.createdAt)}
            ...
            {/* rest all blogs hear */}
            <div className='rounded-2xl grid grid-cols-1 gap-3 md:h-[100vh] md:overflow-y-scroll md:pb-2 md:px-4'>
  {blogs?.slice(2).map((blog) => (
    <div 
      key={blog._id} 
      className='transform transition-transform duration-200 ease-in-out hover:scale-102 md:hover:scale-105'  // Zoom effect only on md and larger
    >
      <BlogCard {...blog} />
    </div>
  ))}
